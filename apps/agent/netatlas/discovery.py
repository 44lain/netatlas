...
def _parse_nmap_grepable_hosts(output: str) -> set[str]:
    hosts: set[str] = set()
...
def discover_hosts(network: str, resolve_hostnames: bool = True) -> list[DiscoveredHost]:
    ...
    hosts = _parse_nmap_grepable_hosts(result.stdout)
    arp_table = _load_arp_table()

    discovered: list[DiscoveredHost] = []
    for ip in sorted(hosts, key=lambda value: tuple(map(int, value.split(".")))):
